...
async def ytdl(_, message):
    ...
    try:
        if userLastDownloadTime > datetime.now():
            ...
            await message.reply_text(f"`Tunggu {wait_time} Menit Ya, Agar Server Tidak Rusak`")
            return
    except:
        pass
    ...
    try:
        ...
        await message.reply_text("`Gagal˜” \nMungkin Youtube Telah Block Ip Heroku, Coba Lagi / Deploy Ulang \n#error`")
        return
    buttons = InlineKeyboardMarkup(list(create_buttons(formats)))
    sentm = await message.reply_text("Tunggu..")
    try:
